...
<div class="directory-grid">{% assign rarity_entries = site.entries | where_exp:'e','e.number >= "126"' | where_exp:'e','e.number <= "150"' | sort:'number' %}{% for e in rarity_entries %}<a class="directory-item" href="{{ e.url | relative_url }}"><span class="num">ENTRY {{ e.number }}</span><strong>{{ e.title }}</strong><small>→</small></a>{% endfor %}</div>
